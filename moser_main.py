...
class CircleScene(Scene):
    args_list = [(RADIANS[:x],) for x in range(1, len(RADIANS) + 1)]
    ...
    def generate_intersection_dots(self):
        ...
        self.intersection_dots = [Dot(point) for point in self.intersection_points]
    ...
    def chop_lines_at_intersection_points(self):
        ...
        for point_pair in it.combinations(self.points, 2):
            int_points = [
                p for p in self.intersection_points if is_on_line(p, *point_pair)
            ]
    ...
    def chop_circle_at_points(self):
        ...
        for i in range(len(self.points)):
            ...
            transform = np.array(
                [[pp[0][0], pp[1][0], 0], [pp[0][1], pp[1][1], 0], [0, 0, 1]],
            )
            circle = deepcopy(self.circle)
            smaller_circle = deepcopy(self.circle)
            for c in circle, smaller_circle:
                c.points = np.dot(c.points, np.transpose(np.linalg.inv(transform)))
                c.filter_out(lambda p: p[0] < 0 or p[1] < 0)
                if c == smaller_circle:
                    c.filter_out(lambda p: p[0] > 4 * p[1] or p[1] > 4 * p[0])
                c.points = np.dot(c.points, np.transpose(transform))
...
class MoserPattern(CircleScene):
    ...
    def __init__(self, radians, *args, **kwargs):
        ...
        for n in range(1, len(radians) + 1):
            ...
            self.add(
                *[Line(p[0], p[1]) for p in it.combinations(self.get_points()[:n], 2)]
            )
            tex_stuffs = [
...
class HardProblemsSimplerQuestions(Scene):
    def __init__(self, *args, **kwargs):
        ...
        self.add(fermat["n"])
        self.play(*[Transform(deepcopy(fermat["n"]), f_copy) for f_copy in copies])
        ...
        other_grid = Mobject(Grid(radius=2), Tex(r"\mathds{C}").shift((2, -2, 0)))
        ...
        self.remove(*self.mobjects)
        self.play(
            Transform(all_mobjects, Point((FRAME_X_RADIUS, 0, 0))),
            Transform(
...
class CountIntersectionPoints(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        formula, answer = Tex(
            [
                r"{%d \choose 4} = \frac{%d(%d - 1)(%d - 2)(%d-3)}{1\cdot 2\cdot 3 \cdot 4}="
                % (n, n, n, n, n),
                str(choose(n, 4)),
            ],
        ).split()
    ...
    def __init__(self, *args, **kwargs):
        ...
        for radians, words, pairs in tuples:
            ...
            if words == "Okay":
                ...
                self.play(
                    *[ShowCreation(line, run_time=1.0) for line in intersecting_lines]
                )
                if first_time:
                    self.play(Transform(Mobject(*intersecting_lines), words_mob))
                    first_time = False
                self.wait()
            self.remove(*self.mobjects)
...
class LineCorrespondsWithPair(CircleScene):
    args_list = [(RADIANS, 2, 5), (RADIANS, 0, 4)]

    @staticmethod
    def args_to_string(*args):
        return int_list_to_string(args[1:])

    def __init__(self, radians, dot0_index, dot1_index, *args, **kwargs):
        CircleScene.__init__(self, radians, *args, **kwargs)
        ...
        line, dot0, dot1 = (
            ...
            self.dots[dot0_index],
            self.dots[dot1_index],
        )
        self.lines.remove(line)
        self.dots.remove(dot0)
        self.dots.remove(dot1)
        self.wait()
        self.play(*[ApplyMethod(mob.fade, 0.2) for mob in self.lines + self.dots])
        self.play(
            *[
                Transform(
                    dot,
                    Dot(dot.get_center(), 3 * dot.radius),
...
class IllustrateNChooseK(Scene):
    args_list = [(n, k) for n in range(1, 10) for k in range(n + 1)]
    ...
    def __init__(self, n, k, *args, **kwargs):
        ...
        form1, count, form2 = Tex(
            [str1, "%d" % choose(n, k), r" \text{ total %s}" % tuple_term],
        )
        pronunciation = TexText("(pronounced ``%d choose %d'')" % (n, k))
        ...
        for mob in tuple_mobs:
            mob.scale(0.6)
        pronunciation.shift(form1.get_center() + (0, 1, 0))
...
class IntersectionPointCorrespondances(CircleScene):
    ...
    def __init__(self, radians, indices, *args, **kwargs):
        ...
        dots_to_save = [self.dots[p] for p in indices]
    ...
    def __init__(self, radians, indices, *args, **kwargs):
        ...
        self.play(
            *[
                Transform(
                    Line(
                        self.get_points()[indices[p0]],
                        self.get_points()[indices[p1]],
                    ),
...
class QuadrupletsToIntersections(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        for quad in quadruplets:
            ...
            self.play(
                Transform(dot_quad, intersection_dot, run_time=3 * frame_time / 2),
            )
...
class GraphsAndEulersFormulaJoke(Scene):
    def __init__(self, *args, **kwargs):
        ...
        self.play(CounterclockwiseTransform(deepcopy(graph), eulers))
        self.wait()
        self.remove(*self.mobjects)
        self.add(eulers)
        self.play(
...
class EulersFormula(GraphScene):
    def __init__(self, *args, **kwargs):
        GraphScene.__init__(self, *args, **kwargs)
        terms = "V - E + F =2".split(" ")
        form = dict([(key, mob) for key, mob in zip(terms, Texs(terms))])
        ...
        self.add(formula)
        colored_dots = [deepcopy(d).scale(1.5).set_color("red") for d in self.dots]
        colored_edges = [
            Mobject(
                Line(midpoint, start),
                Line(midpoint, end),
            ...
            for e in self.edges
            for start, end, midpoint in [(e.start, e.end, (e.start + e.end) / 2.0)]
        ]
        frame_time = 0.3
...
class CannotDirectlyApplyEulerToMoser(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        for mob in n_equals, intersection_count:
            mob.shift(shift_val)
        self.add(n_equals)
        yellow_dots = [d.set_color("yellow").scale(2) for d in deepcopy(self.dots)]
        yellow_lines = Mobject(*[l.set_color("yellow") for l in deepcopy(self.lines)])
        self.play(*[ShowCreation(dot) for dot in yellow_dots], run_time=1.0)
        self.wait()
        self.remove(*yellow_dots)
        self.play(ShowCreation(yellow_lines))
        self.wait()
        self.remove(yellow_lines)
        cannot_intersect = TexText(
            r"""
...
class ShowMoserGraphLines(CircleScene):
    def __init__(self, radians, *args, **kwargs):
        ...
        small_lines = [deepcopy(line).scale(0.5) for line in self.lines]
        ...
        yellow_lines = Mobject(*[line.set_color("yellow") for line in small_lines])
        ...
        self.play(
            ShowCreation(
                Mobject(*[mob.set_color("yellow") for mob in self.circle_pieces]),
            ),
        )
...
class HowIntersectionChopsLine(CircleScene):
    ...
    def __init__(self, radians, indices, *args, **kwargs):
        ...
        self.add(*lines)
        self.play(*[FadeOut(mob) for mob in self.mobjects if mob not in lines])
        new_lines = [Line(line.start, intersection_point) for line in lines] + [
            Line(intersection_point, line.end) for line in lines
        ]
        self.play(
            *[
                Transform(
        ...
        self.play(
            *[
                Transform(line, deepcopy(line).scale(1.1).scale(1 / 1.1), run_time=1.5)
...
class ApplyEulerToMoser(CircleScene):
    def __init__(self, *args, **kwargs):
        ...
        V[1], minus[1], E[1], plus[1], F[1], equals[1], two[1] = Tex(
            ["V", "-", "E", "+", "F", "=", "2"],
        ).split()
        F[2], equals[2], E[2], minus[2], V[2], plus[2], two[2] = Tex(
            ["F", "=", "E", "-", "V", "+", "2"],
        ).split()
        F[3], equals[3], E[3], minus[3], n[3], minus1[3], nc4[3], plus[3], two[3] = Tex(
            ["F", "=", "E", "-", "n", "-", r"{n \choose 4}", "+", "2"],
        ).split()
        (
            F[4],
            equals[4],
        ...
        F[6], equals[6], nc2[6], plus1[6], nc4[6], plus[6], two[6] = Tex(
            ["F", "=", r"{n \choose 2}", "+", r"{n \choose 4}", "+", "2"],
        ).split()
        F[7], equals[7], two[7], plus[7], nc2[7], plus1[7], nc4[7] = Tex(
            ["F", "=", "2", "+", r"{n \choose 2}", "+", r"{n \choose 4}"],
        ).split()
        shift_val = (0, 3, 0)
        for d in dicts:
            if not d:
            ...
            for key in list(d.keys()):
                d[key] = deepcopy(d[main_key]).shift(d[key].get_center() - main_center)
        ...
        self.play(
            *[
            ...
            + [
                Transform(deepcopy(cp), scp, run_time=2.0)
        ...
        self.play(
            *[
                Transform(deepcopy(dot), deepcopy(dot).scale(1.4).set_color("yellow"))
                for dot in self.dots + self.intersection_dots
            ]
            + [
                Transform(deepcopy(line), deepcopy(line).fade(0.4))
        ...
        self.play(
            ...
            *[Transform(d[2], d[3]) for d in [F, equals, E, minus, plus, two]]
        )
        self.wait()
        self.remove(*self.mobjects)
        self.play(
            Transform(
                E[3],
                Mobject(nc2[4], plus1[4], two1[4], nc41[4], plus2[4], n1[4]),
            ),
            *[
                Transform(d[3], d[4])
        ...
        self.play(
            ...
            *[
                Transform(d[4], d[5])
                for d in [F, equals, nc2, plus1, two1, nc41, minus1, nc4, plus, two]
            ]
        )
        self.wait()
        self.remove(*self.mobjects)
        self.play(
            ...
            *[Transform(d[5], d[6]) for d in [F, equals, nc2, plus1, nc4, plus, two]]
        )
        self.wait()
        self.remove(*self.mobjects)
        self.play(
            ...
            *[Transform(d[6], d[7]) for d in [F, equals, nc2, plus1, nc4]]
        )
        self.wait()
        self.add(*self.lines + self.circle_pieces)
        for region in self.regions:
...
class FormulaRelatesToPowersOfTwo(Scene):
    def __init__(self, *args, **kwargs):
        ...
        self.add(*forms)
        self.play(*[FadeIn(s) for s in sums])
        self.wait()
        self.play(*[Transform(deepcopy(s), result) for s, result in zip(sums, results)])
        powers_of_two = [
            Tex("2^{%d}" % (i - 1))
            .scale(scale_factor)
    ...
    def __init__(self, nrows, n, k, *args, **kwargs):
        ...
        self.play(
            *[ShowCreation(mob) for mob in triangle_terms]
            + [ApplyMethod(mob.shift, formula_center) for mob in formula_terms],
            run_time=1.0
        )
...
class MoserSolutionInPascal(PascalsTriangleScene):
    args_list = [(N_PASCAL_ROWS, n) for n in range(3, 8)] + [(BIG_N_PASCAL_ROWS, 10)]
    ...
    def __init__(self, nrows, n, *args, **kwargs):
        ...
        for k in range(len(terms)):
            if k % 2 == 0 and k <= n:
                ...
                new_term = Point(self.coords_to_center(n, k))
        ...
        self.play(
            *[
            ...
            + [CounterclockwiseTransform(terms[k], target_terms[k]) for k in term_range]
        )
        self.wait()
        for k in term_range:
            if k == 0:
            ...
            self.play(Transform(terms[k], Mobject(*above_terms).set_color(term_color)))
...
class RotatingPolyhedra(Scene):
    ...
    def __init__(self, polyhedra_classes, *args, **kwargs):
        Scene.__init__(self, *args, **kwargs)
        rot_kwargs = {"radians": np.pi / 2, "run_time": 5.0, "axis": [0, 1, 0]}
        polyhedra = [Class().scale(1.5).shift((1, 0, 0)) for Class in polyhedra_classes]
...
class ExplainNChoose2Formula(Scene):
    ...
    def __init__(self, n, a, b, *args, **kwargs):
        ...
        n_mob, n_minus_1, over_2 = Tex([str(n), "(%d-1)" % n, r"\over{2}"]).split()
        ...
        right_shift[1] = 0
        self.play(ApplyMethod(nums_compound.shift, right_shift), FadeIn(n_minus_1))
        ...
        self.play(
            *[ApplyMethod(mob.shift, (0, 1, 0)) for mob in (parens, a_mob, b_mob)]
        )
...
class ExplainNChoose4Formula(Scene):
    ...
    def __init__(self, n, *args, **kwargs):
        ...
        quad = (4, 2, 5, 1)
        tuple_mobs = Texs(("( %d , %d , %d , %d )" % quad).split(" "))
        ...
        for i in range(4):
            ...
            self.play(ApplyMethod(nums_compound.shift, (curr_num - quad[i]) * up_unit))
            self.remove(nums_compound)
            nums = nums_compound.split()
            ...
            if i < 3:
                ...
                right_shift[1] = 0
                self.play(ApplyMethod(nums_compound.shift, right_shift))
            else:
                self.play(
                    *[
                        CounterclockwiseTransform(
        ...
        for count in range(6):
            ...
            new_quad_mobs = [
                deepcopy(quad_mobs[i]).shift(
                    quad_mobs[perm[i]].get_center() - quad_mobs[i].get_center(),
                )
                for i in range(4)
            ]
            compound_quad = Mobject(*quad_mobs)
            self.play(CounterclockwiseTransform(compound_quad, Mobject(*new_quad_mobs)))
...
class IntersectionChoppingExamples(Scene):
    def __init__(self, *args, **kwargs):
        ...
        for pairs, exp in [(pairs1, "3 + 2(2) = 7"), (pairs2, "4 + 2(3) = 10")]:
