    ...
    def construct(self):
        ...
        self.play(
            *[
            ...
            + [
                Transform(
                    Mobject(*[self.vertices[index] for index in cycle]), dv, **kwargs
                )
...
class IntroduceGraph(GraphScene):
    def construct(self):
        ...
        connected, planar, graph = (
            TexText(["Connected ", "Planar ", "Graph"]).to_edge(UP).split()
        )
...
class PlanarGraphDefinition(Scene):
    def construct(self):
        ...
        graphs = [
            ...
            for g in [CubeGraph(), CompleteGraph(5), OctohedronGraph()]
        ]
...
class TerminologyFromPolyhedra(GraphScene):
    ...
    def construct(self):
        ...
        cube = Mobject(
            *[Line(vertices[edge[0]], vertices[edge[1]]) for edge in self.graph.edges]
        )
        ...
        self.play(ApplyMethod(Mobject(*self.edges).set_color, "yellow"))
        self.wait(2)
        self.clear()
        self.add(*self.edges)
...
class IntroduceCycle(WalkingRandolph):
    args_list = [(SampleGraph(), [0, 1, 3, 2, 0])]
    ...
    def construct(self):
        ...
        regions = [self.region_from_cycle(cycle) for cycle in encompassed_cycles]
    ...
    def construct(self):
        ...
        self.play(
            Transform(
                randy,
                deepcopy(randy)
                .scale(RANDOLPH_SCALE_FACTOR)
                .move_to(self.get_points()[0]),
            ),
        )
        self.wait()
        name.shift((0, 1, 0))
...
class DefineSpanningTree(GraphScene):
    def construct(self):
        ...
        dollar_signs = Mobject(
            *[deepcopy(dollar_signs).shift(edge.get_center()) for edge in self.edges]
        )
        ...
        def out_of_spanning_set(point_pair):
            ...
            return point_pair not in stip and tuple(reversed(point_pair)) not in stip
        ...
        self.play(ShowCreation(green_dot_at_index(0)), run_time=run_time_per_branch)
        for pair in self.spanning_tree_index_pairs:
            ...
            self.play(
                ShowCreation(green_dot_at_index(pair[1]), run_time=run_time_per_branch),
            )
        self.wait(2)
...
class DualGraph(GraphScene):
    def construct(self):
        ...
        self.play(*[ShowCreation(mob) for mob in self.dual_edges + self.dual_vertices])
        self.wait()
...
class FacebookGraph(GraphScene):
    def construct(self):
        ...
        friends = Tex("\\leftarrow \\text{friends} \\rightarrow").scale(
...
class FacebookGraphAsAbstractSet(Scene):
    def construct(self):
        ...
        friends_mob = Tex(friends_string, size="\\Large").shift(3 * RIGHT)
        ...
        for mob in names_mob, friends_mob:
            self.play(ShowCreation(mob, run_time=1.0))
        self.wait()
...
class ExamplesOfGraphs(GraphScene):
    def construct(self):
        ...
        self.generate_regions()
        objects, notions = (
            Mobject(*TexText(["Objects \\quad\\quad ", "Thing that connects objects"]))
            .to_corner()
            .shift(0.5 * RIGHT)
            .split()
        )
        ...
        objects_and_notions = [
            ...
            ("Neurons", "Synapses"),
            ("Regions our graph \\\\ cuts the plane into", "Shared edges"),
        ]
    ...
    def handle_dual_graph(self, words1, words2):
        ...
        self.play(
            *[ShowCreation(mob, run_time=1.0) for mob in self.edges + self.vertices]
        )
        self.wait()
        for region in self.regions:
            self.set_color_region(region)
...
class DrawDualGraph(GraphScene):
    def construct(self):
        ...
        internal_edges = [
            e
            for e in self.dual_edges
            if abs(e.start[0]) < FRAME_X_RADIUS
            and abs(e.end[0]) < FRAME_X_RADIUS
            and abs(e.start[1]) < FRAME_Y_RADIUS
    ...
    def construct(self):
        ...
        self.play(
            *[
                Transform(
                    v,
                    Dot(v.get_center(), radius=3 * Dot.DEFAULT_RADIUS).set_color(
                        "green",
                    ),
                )
...
class IntroduceMortimer(GraphScene):
    ...
    def construct(self):
        ...
        self.play(
            *[
                ApplyMethod(line.rotate, np.pi / 10, rate_func=wiggle)
        ...
        self.play(
            ShowCreation(
                Mobject(
                    *[
                        Line(
                            self.get_points()[last],
                            self.get_points()[next],
                        ).set_color("green")
    ...
    def construct(self):
        ...
        self.play(
            *[
                Transform(
                    v,
                    Dot(v.get_center(), radius=3 * Dot.DEFAULT_RADIUS, color="green"),
                )
...
class TwoPropertiesOfSpanningTree(Scene):
    def construct(self):
        spanning, tree = TexText(["Spanning ", "Tree"], size="\\Huge").split()
...
class DualSpanningTree(GraphScene):
    def construct(self):
        ...
        self.play(
            ShowCreation(Mobject(*np.array(self.edges)[dual_edges]).set_color("red")),
        )
        self.add(words)
        self.wait()
...
class FinalSum(Scene):
    def construct(self):
        ...
        symbols = V, minus, E, plus, F, equals, two = Tex("V - E + F = 2".split(" "))
        plus = Tex("+")
        anims = []
